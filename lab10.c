...
1. How is the client sending data to the server? What protocol?
   The client is sending data using TCP over IPv4.

2. What data is the client sending to the server?
   The client is sending 5 messages "Hello", "Apple", "Car", "Green", Dog"
   written in a buffer.

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
   run_acceptor is passed a struct that contains a boolean flag that controls
when the while loop ends. Once it is set to false, the run_acceptor will stop
accepting clients. It is also passed a list_handle, a struct that contains a
linked list of data. Also there is a mutex lock that protects the linked lsit
for thread safety.

2. How are received messages stored?
   Stored in a LinkedList defined by list_node and access by struct list_handle.
   when a message is recieved, a new struct list_node is created. The mutex is
locked, and the new node is added to list_handle, it also keeps track of the
count.

3. What does `main()` do with the received messages?
   Main collects the recieved messages and counts them ensuring that all
messages that were supposed to be sent.

4. How are threads used in this sample code?
   Threads are used to handle the multiple clients connecting at the same time.
Main creates an acceptor thread that listens for connections and accepts them
(up to 4 clients). run_accepter also creates threads for each client.

Additional Questions:
1. How are sockets made non-blocking?
   using set_non_blocking(fd), fd is the socket

2. What sockets are made non-blocking?
   The server socker is made non-blocking in run_acceptor in the line that says:
   set_non_blocking(sfd);

   The client socket is also non-block in run_client: set_non_blocking(cfd);

3. Why are these sockets made non-blocking? What purpose does it serve?
   Instead of blocking (waiting for connection), the socket can check at a
certain interval. The length at which the socket searches for a connection is
determined by the run flag that controls a while loop. Using a non-blocking
socket allows the program not to get stuck waiting for a connection and allows
it to stop searching whenever it wants using the flag.
*/
