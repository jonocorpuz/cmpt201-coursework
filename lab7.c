...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  int doubledVal = input->doubled_value;

  for (int i = 0; i < *result_count; i++) {
    if (doubledVal == output[i].doubled_value) {
      output[i].line_numbers[output[i].count] = input->line_number;
      output[i].count++;
    }
  }
  output[*result_count].doubled_value = doubledVal;
...
void reduce(Output output) {
  ...
  printf("%d", output.line_numbers[output.count]);
  printf("])\n");
